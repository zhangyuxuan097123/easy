...
st.markdown(
    ...
    .topo-node { 
        ...
        font-weight: bold; font-size: 0.95rem; /* 字體稍微縮小以容納3個中文字 */
...
with tab_dashboard:
    try:
        ...
        for _, row in source_df.iterrows():
            s_id = int(row['Station'])
            caps = parse_list_from_string(row['capacities'])
            probs = parse_list_from_string(row['probs'])
            
            # 使用 MAP 自動轉換中文名稱，若沒有對應則保留預設
            STATION_DATA.append({
                "name": STATION_NAME_MAP.get(s_id, f"工作站 {s_id}"), 
                "id": s_id,
                "capacities": caps,
                "probs": probs,
                "p": float(row['p']),
    ...
    if not STATION_DATA:
        ...
        for i, col in enumerate(topo_cols):
            with col:
                ...
                # 顯示中文站名
                html_content += f"""<div class="topo-node {node_states[i]}">{STATION_DATA[i]["name"]}</div>"""
        ...
        x_margin = max(step, (max_x_val - min_x_val) * 0.15) 
        ...
        fig3.update_layout(
            ...
            xaxis=dict(
                ...
                range=[min_x_val - x_margin, max_x_val + x_margin] 
            ),
            yaxis=dict(
                title_font=dict(size=18, color='#000000', family='Arial'), 
                color='#000000', linecolor='#000000', linewidth=1, 
...
with tab_editor:
    ...
    edited_df = st.data_editor(
        ...
        disabled=["k"], 
        column_config={
            "Station": st.column_config.NumberColumn("站號 (1:吹瓶 2:充填 3:套標 4:包裝 5:疊棧)", min_value=1, step=1, required=True),
