            ...
            try:
                ...
                try:
                    ...
                    reply_match = re.search(
                        r'["\']?reply["\']?\s*:\s*["\']?(.*?)(?:,?\s*["\']?chart["\']?\s*:|$)',
                        raw, re.DOTALL | re.IGNORECASE
                    )
                    if reply_match:
                        reply_text = re.sub(r'["\'\}]+$', '', reply_match.group(1).strip())
                        reply_text = reply_text.replace('\\n', '\n').replace('\n', '<br>')
                    else:
                        reply_text = re.sub(
                            r'["\']?[dtbcfchart]+["\']?\s*:\s*[A-Za-z0-9\.\-\_]*\s*,?',
...
with tab_chat:
    ...
    if not st.session_state.chat_history:
        ...
        for turn in st.session_state.chat_history:
            ...
            if turn.get("chart"):
                ...
                chart_metrics  = calculate_metrics(s["d"], s["cf"], STATION_DATA_CHAT, s["tb"])
                station_labels = ["吹瓶站", "充填站", "套標站", "包裝站", "疊棧站"]
    ...
    if final_query:
        if not GOOGLE_API_KEYS:
            st.error("⚠️ 請先設定 GOOGLE_API_KEY_1 環境變數！")
            ...
            with st.spinner("🤖 AI 戰情助理分析中..."):
                try:
                    ...
                    extracted, ai_reply, ai_chart = call_ai_single(
                        final_query, current_params, current_metrics,
                        st.session_state.chat_history
                    )
                    sim_changed = (
            ...
            try:
                ...
                for row, caps, probs in validated_rows:
                    for i in range(len(caps)):
                        ...
                        long_rows.append({
                            "Station": int(row['Station']), "Machine": 1,
                            "Success rate": row['p'], "Power(kW)加工功率": row['power'],
                            "capacity": caps[i], "Capacity_Prob": probs[i],
